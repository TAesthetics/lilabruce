...
class TempleDeck {
  ...
  enterDeck(username) {
    ...
    this.poll();
    this._poll = setInterval(() => this.poll(), 2200);
  ...
  updateEngMeta(eng) {
    ...
    if (!el) return;
    if (!eng) {
      el.textContent = 'Create an engagement to scope work to a client/job.';
      return;
    }
    el.textContent = [eng.client && `Client: ${eng.client}`, eng.scope && `Scope: ${eng.scope}`]
      .filter(Boolean).join(' · ') || eng.name;
  }
  ...
  async createEngagement() {
    ...
    this.closeEngModal();
    this.termPrint('success', `Engagement: ${name}`);
  ...
  async generateReport() {
    ...
    try {
      ...
      if (res.error) this.termPrint('err', res.error);
        ...
        this.termPrint('success', 'Report ready');
  ...
  async exec(cmd) {
    ...
    if (c === '/status') {
      await this.poll();
      this.termPrint('success', 'ok');
      return;
    }
    if (c === '/target') {
      if (!arg) {
        this.termPrint('info', this.getTarget());
        return;
      }
      document.getElementById('target-input').value = arg;
      this.termPrint('success', '→ ' + arg);
      return;
    }
    if (c === '/loop') {
      if (arg === 'start') return this.startLoop();
      if (arg === 'stop') return this.stopLoop();
      this.termPrint('err', 'start | stop');
      return;
    }
  ...
  async startLoop() {
    this.termPrint('cmd', '▶ ' + this.getTarget());
  ...
  async runAgent(name, override) {
    if (this.busy) {
      this.termPrint('err', 'busy');
      return;
    }
    this.busy = true;
    const target = override || this.getTarget();
    this.termPrint('cmd', `${name} → ${target}`);
    ...
    try {
      ...
      if (res.error) {
        ...
        this.termPrint('success', name + ' ok');
  ...
  async runTool(tool) {
    ...
    this.termPrint('cmd', `${tool} → ${target}`);
    try {
      const res = await this.api('POST', '/api/tool/' + tool, { target });
      if (res.error) this.termPrint('err', res.error);
      else {
        const text = typeof res.content === 'string' ? res.content : JSON.stringify(res.content);
        text.split('\n').slice(0, 6).forEach(l => this.termPrint('father', l));
        if (text.split('\n').length > 6) this.termPrint('info', '… full in RESULTS');
        this.showResult(res);
      }
    } catch (e) {
      this.termPrint('err', e.message);
  ...
  switchTab(tab) {
    ...
    if (tab === 'report') {
      this.showResult({ content: this.lastReport || 'No report yet.' });
      return;
    }
    this.loadResult(tab);
  }
  ...
  renderStatus(d) {
    ...
    if (ph) {
      ph.textContent = !d.current_phase || d.current_phase === 'idle'
        ? '—'
        : d.current_phase.replace('_complete', ' ✓').toUpperCase();
    }
  ...
  async saveKey() {
    ...
    try {
      ...
      this.termPrint('success', key ? 'key saved' : 'key cleared');
    } catch (e) {
  ...
  async testKey() {
    ...
    try {
      ...
      if (res.error) {
        ...
        st.textContent = 'Father present.';
        st.className = 'key-status ok';
        this.termPrint('success', 'Father ok');
      }
    } catch (e) {
      st.textContent = e.message;
      st.className = 'key-status err';
